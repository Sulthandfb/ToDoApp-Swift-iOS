...
//  L25020028-ToDoApp
//
//  Design: Clean soft-blue modern UI inspired by GoTask
//
            ...
            HStack(alignment: .center) {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    HStack(spacing: -8) {
                        ForEach(0..<3, id: \.self) { i in
                            Circle()
                                .fill([Color.orange, Color.green, Color.pink][i])
                ...
                VStack(alignment: .trailing, spacing: 8) {
                    ...
                    ZStack(alignment: .leading) {
                        ...
                        Capsule()
                            .fill(Color.white)
                            .frame(width: max(6, 90 * progress), height: 6)
                    }
                }
            }
            ...
            HStack(spacing: 8) {
                ForEach(FilterTab.allCases, id: \.self) { tab in
                    Button {
                        ...
                        Text(tab.rawValue)
                            .font(.system(size: 13,
                                          weight: selectedFilter == tab ? .semibold : .regular,
                                          design: .rounded))
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                Text(item.title)
                    ...
                    .strikethrough(item.isDone, color: Color.textSecondary)
                    .lineLimit(1)
                HStack(spacing: 4) {
            ...
            VStack(spacing: 8) {
                ...
                Button(action: onToggle) {
                    ZStack {
                        Circle()
                            .strokeBorder(item.isDone ? categoryColor : Color.gray.opacity(0.3), lineWidth: 2)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 18)
                .stroke(item.isDone ? categoryColor.opacity(0.2) : Color.clear, lineWidth: 1)
        )
    }
}
...
struct ToDoDetailView: View {
    ...
    var body: some View {
                ...
                VStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: 6) {
                            ...
                            Text(editedDueDate.formatted(date: .long, time: .shortened))
                                .font(.system(size: 13, design: .rounded))
                        }
                        .foregroundColor(Color.textSecondary)
                    }
                        ...
                        Button(action: saveChanges) {
                            Text("Save Changes")
                                ...
                                .background(LinearGradient(
                                    ...
                                    startPoint: .leading, endPoint: .trailing))
                                .cornerRadius(16)
                                .shadow(color: Color.brandBlue.opacity(0.4), radius: 12, x: 0, y: 6)
                        }
        ...
        .alert("Delete Task?", isPresented: $showDeleteAlert) {
            Button("Delete", role: .destructive) {
                if let id = item.id { viewModel.delete(id: id) }
                dismiss()
            }
    ...
    private func saveChanges() {
        ...
        updated.detail   = editedDetail
        updated.dueDate  = editedDueDate
        updated.isDone   = editedIsDone
                    ...
                    VStack(spacing: 16) {
                                ...
                                HStack(spacing: 8) {
                                    ForEach(ToDoItem.Priority.allCases, id: \.self) { p in
                                        Button {
                                            ...
                                            Text(p.rawValue)
                                                .font(.system(size: 13,
                                                              weight: priority == p ? .semibold : .regular,
                                                              design: .rounded))
                                                .foregroundColor(priority == p ? .white : Color.textSecondary)
                                                .padding(.horizontal, 18).padding(.vertical, 10)
                                                .background(priority == p ? Color.brandBlue : Color.bgPage)
                                                .cornerRadius(10)
                                        }
                                    }
                                }
                        ...
                        formCard {
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                TextField("Add details about this task...",
                        ...
                        Button {
                            ...
                            viewModel.add(ToDoItem(title: title, detail: detail,
                                                   isDone: isDone, dueDate: dueDate, priority: priority))
                            dismiss()
                        } label: {
                            Text("Create Task")
                                .font(.system(size: 16, weight: .semibold, design: .rounded))
